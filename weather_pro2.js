...
const provider = {
    ...
    darksky: {
        api: function () {
            //`https://api.darksky.net/forecast/${config.darksky_api}/${config.lat_lon.replace(/\s/g, "").replace("，", ",")}?lang=${config.lang}&units=si`
            var url = `https://api.darksky.net/forecast/${config.darksky_api}/${getLocation().replace(/\s/g, "").replace("，", ",")}?lang=${config.lang}&units=si`
    ...
    aqicn: {
        api: function () {
            // `https://api.waqi.info/feed/geo:${config.lat_lon.replace(/\s/g, "").replace("，", ",").replace(/,/, ";")}/?token=${config.aqicn_api}`
            var url = `https://api.waqi.info/feed/geo:${getLocation().replace(/\s/g, "").replace("，", ",").replace(/,/, ";")}/?token=${config.aqicn_api}`
            console.log(`aqicn url:${url}`)
            return url
        },
        progress: 0,
        data: {},
...
function isAutoLocation() {
    return config.lat_lon.startsWith("auto")
}

function getLocation() {
    var location = isAutoLocation() ? `${provider.heweather_now.data.basic.lat},${provider.heweather_now.data.basic.lon}` : config.lat_lon
    return location
}
